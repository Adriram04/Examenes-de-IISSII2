...
const update = async function (req, res) {
  try {
    ...
    for (const produto of productos) {
      const newPrice = produto.basePrice + ((produto.basePrice * req.body.percentage) / 100)
      await produto.update({ price: newPrice }, transaction)
    }
    await transaction.commit()

    const updateRestaurant = await Restaurant.findByPk(req.params.restaurantId)
